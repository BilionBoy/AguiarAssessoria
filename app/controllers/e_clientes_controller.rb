...
class EClientesController < ApplicationController
  ...
  def set_e_cliente
    ...
    return redirect_to e_clientes_path, alert: t('messages.not_found') unless @e_cliente
  end

  def e_cliente_params
    permitted_attributes = ECliente.column_names.reject { |col| ['deleted_at', 'created_by', 'updated_by'].include?(col) }
